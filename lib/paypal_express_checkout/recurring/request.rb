module PaypalExpressCheckout
  module Recurring
    class Request
      ...
      #
      #   response = request.run(:checkout)
      #
      # We normalize the methods name. For a list of what's being covered, refer to
      # PaypalExpressCheckout::Recurring::Request::METHODS constant.
      #
      # The params hash can use normalized names. For a list, check the
      # PaypalExpressCheckout::Recurring::Request::ATTRIBUTES constant.
      #
      ...
      def request
        @request ||= Net::HTTP::Post.new(uri.request_uri).tap do |http|
          http["User-Agent"] = "PayPal::Recurring/#{PaypalExpressCheckout::VERSION}"
        end
      end
      ...
      #
      def uri # :nodoc:
        @uri ||= URI.parse(PaypalExpressCheckout::Recurring.api_endpoint)
      end
      ...
      def default_params
        {
            :username    => PaypalExpressCheckout::Recurring.username,
            :password    => PaypalExpressCheckout::Recurring.password,
            :signature   => PaypalExpressCheckout::Recurring.signature,
            :version     => PaypalExpressCheckout::Recurring.api_version
        }
      end
