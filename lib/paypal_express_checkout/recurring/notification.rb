module PaypalExpressCheckout
  module Recurring
    class Notification
      extend PaypalExpressCheckout::Recurring::Utils
      ...
      def request
        @request ||= PaypalExpressCheckout::Recurring::Request.new.tap do |request|
          request.uri = URI.parse("#{PaypalExpressCheckout::Recurring.site_endpoint}?cmd=_notify-validate")
        end
      end
      ...
      def valid?
        completed? && verified? && email == PaypalExpressCheckout::Recurring.email && seller_id == PaypalExpressCheckout::Recurring.seller_id
      end
